...
return (function(){ var percival;
  ...
  var hardEscape = function (str) { var out;
    ...
    str.match(new(RegExp)("[^"+percival.escapes.join('')+"]+|(?:.|\n)", 'g'))
      .forEach(function (subStr) { var matchings;
        if (matchings = subStr.match(
          ...
          switch (matchings[0]) {
            case matchings[2]:
              ...
              break;
            case matchings[1]: subStr = 'n';
            default:
              out.push(yarn.beget({ elements : ['\\', subStr] }).label('escape') );
          }
        } else { out.push(subStr) }
      });
